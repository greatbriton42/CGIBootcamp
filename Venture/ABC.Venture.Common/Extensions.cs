...
using ABC.Venture.Common.Exceptions;
...
namespace ABC.Venture.Common
{
   ...
   public static class Extensions
   {
        ...
        /// <seealso cref="DynamicGrid{T}"/>
        public static IGrid<T> ToPagedList<T>(this IQueryable<T> superset, string filter, 
                                              string sort, string order, int index = 1, int pageSize = Constants.MAX_RECORDS_PER_TABLE)
      {
         ...
         return new DynamicGrid<T>(superset, filter, sort, order, index, pageSize);
      }

      public static IGrid<U> ConvertCollection<T,U>(this IGrid<T> oldCollection, IEnumerable<U> newCollection)
      {
         return new DynamicGrid<U>(newCollection, oldCollection.Criteria);
      }
      ...
      /// <summary>
      /// Take a string and return the equivalent projecthealthcolor
      /// </summary>
      /// <param name="stringValue"></param>
      /// <returns></returns>
      public static EnumTypes.ProjectHealthColor ToProjectHealthColor(this string stringValue)
      {
         if(stringValue == null)
         {
            return EnumTypes.ProjectHealthColor.White;
         }

         switch (stringValue.ToLower())
         {
               case "red":
                  return EnumTypes.ProjectHealthColor.Red;
               case "green":
                  return EnumTypes.ProjectHealthColor.Green;
               case "yellow":
                  return EnumTypes.ProjectHealthColor.Yellow;
               default:
                  return EnumTypes.ProjectHealthColor.White;
         }
      ...
      /// <summary>
      /// Take a string and return the equivalent ProjectRatingColor
      /// </summary>
      /// <param name="stringValue"></param>
      /// <returns></returns>
   }
