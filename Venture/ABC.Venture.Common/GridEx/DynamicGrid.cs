...
namespace ABC.Venture.Common.GridEx
{
   ...
   /// <seealso cref="BaseGrid{T}"/>
   /// <seealso cref="StaticPagedList{T}"/>
   /// <seealso cref="List{T}"/>
   public class DynamicGrid<T> : BaseGrid<T>
   {
      /// <summary>
      /// Initializes a new instance of the <see cref="DynamicGrid{T}"/> class that divides the supplied superset into subsets the size of the supplied pageSize. The instance then only containes the objects contained in the subset specified by index.
      /// </summary>
      ...
      public DynamicGrid(IEnumerable<T> superset, string filter, string sort, string order, int index, int pageSize)
          : this(superset == null ? new List<T>().AsQueryable() : superset.AsQueryable(), filter, sort, order, index, pageSize)
      {
      }
      private DynamicGrid(IQueryable<T> superset, string filter, string sort, string order, int index, int pageSize)
          : base(index, pageSize)
      {
      ...
      internal DynamicGrid(IEnumerable<T> newCollection, PageSortCriteria criteria)
         :base(criteria.PageIndex, criteria.PageSize,criteria.TotalItemCount)
      {
         AddRange(newCollection);
      }
   }
}
